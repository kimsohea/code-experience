...
const SiteList = ({ sectionRef, isActive }) => {
  const [activeThumb, setActiveThumb] = useState("vue");
  const [siteIdx, setSiteIdx] = useState(0);
  const [infoFlg, setInfoFlg] = useState(false);

  const siteInfo = useMemo(() => {
    const siteList = { vueSite: vueSite, reactSite: reactSite, javascriptSite: javascriptSite, jquerySite: jquerySite };
  ...
  return (
    <section id="Site_Lists" className={isActive ? "site_list active" : "site_list"} ref={sectionRef}>
      <h3>참여 프로젝트 (단체 + 개인)</h3>
